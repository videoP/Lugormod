...
//Lugormod emote stuff
qboolean
endAnim (int anim) 
{
	int i;
	for (i = 0; emotes[i].name && emotes[i].animEnd != anim; i++);
	return (emotes[i].name != NULL);
}
...
//#define SUPER_JP_LEVEL 4
//end Lugormod
//int Accounts_Prof_GetProfession(gentity_t *ent);
//int MercSkillLevel(gentity_t *ent, int skill);
#endif
...
//Ufo: small fixes
int forcePowerNeeded[NUM_FORCE_POWER_LEVELS + 2][NUM_FORCE_POWERS] = 
{
	...
	{ //1
		50,//FP_HEAL,//instant //was 25, but that was way too little
...
//#ifdef __LCC__
#if (defined __LCC__ || defined __MACOS__)
...
int PM_GetSaberStance(void)
{
	...
	if (saber1) {
		...
		for (b = 0; b < nb; b++) {
			saber1->blade[b].lengthMax = 
				saber1->blade[b].unscaledLengthMax * scaleh;
			saber1->blade[b].radius = 
				saber1->blade[b].unscaledRadius * scaleh;
		}
	}
	if (saber2) {
		nb = saber2->numBlades;
		...
		for (b = 0; b < nb; b++) {
			saber2->blade[b].lengthMax = 
				saber2->blade[b].unscaledLengthMax * scaleh;
			saber2->blade[b].radius = 
				saber2->blade[b].unscaledRadius * scaleh;
		}
	}
...
void PM_pitch_roll_for_slope( bgEntity_t *forwhom, vec3_t pass_slope, vec3_t storeAngles )
{
	...
	else //if ( forwhom->client )
	{
		...
		oldmins2 = pm->mins[2];
		//Lugormod scale here ????
		pm->mins[2] = (-24 + 12 * fabs(pm->ps->viewangles[PITCH])/180.0f);// * scaleh;
...
static void PM_SetVehicleAngles( vec3_t normal )
{
	...
	/*
	if (pVeh->m_pPilot) {
	gentity_t *driver = (gentity_t*)pVeh->m_pPilot;
	if (driver->client && authenticated(driver,1)) {
	trap_SendServerCommand(driver-g_entities,va("cp \"%s %s\"", vtos(vAngles), vtos(pVeh->m_vOrientation)));
	}
	}
	*/
}
...
static void PM_Friction( void ) {
	...
	//Lugormod fix sliding fighters
	if (pEnt && pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle
		&& pEnt->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER
...
static void PM_Accelerate( vec3_t wishdir, float wishspeed, float accel )
{
	...
	// RoboPhred: Try to stop backwards bunny hop to avoid merc flame
	if ((pm->gametype == GT_SIEGE || pm->ps->fd.forcePowersActive & (1 << FP_GRIP)) &&
		pm->ps->clientNum < MAX_CLIENTS &&
		pm->ps->pm_type == PM_NORMAL &&
		!pm->ps->m_iVehicleNum) {
		// Proper non-bunnyhop way
		vec3_t		wishVelocity;
		vec3_t		pushDir;
		float		pushLen;
		float		canPush;
	...
	else { //standard method, allows "bunnyhopping" and whatnot
		int			i;
...
qboolean PM_AdjustAngleForWallRun( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//wall-running and not at end of anim
		...
		//Lugormod scale here ?
		//VectorScale (mins, scaleh, mins);
		//VectorScale (maxs, scaleh, mins);
...
qboolean PM_AdjustAngleForWallJump( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//hugging wall, getting ready to jump off
		...
		else if ( doMove && (ps->pm_flags&PMF_STUCK_TO_WALL))
		{//jump off
			//push off of it!
			ps->pm_flags &= ~PMF_STUCK_TO_WALL;
...
float forceJumpHeightMax[NUM_FORCE_POWER_LEVELS + 2] = 
{
	//Ufo: boosted level 4
	...
	1570,//(1536+stepheight(18)+crouchdiff(24) = 426)
	1570//(1536+stepheight(18)+crouchdiff(24) = 426)
};

#ifdef LMD_NEW_JUMP
//RoboPhred
float forceJumpFallVelocityScaler[NUM_FORCE_POWER_LEVELS + 2] =
{
	0,
	3. / 8,
	4. / 8,
...
{
	int jumpLevel = pm->ps->fd.forcePowerLevel[FP_LEVITATION];
	...
	if ( pm->waterlevel < 2 ) 
	{
		...
		{//can't do this in zero-G
			if ( PM_ForceJumping() ) {
				if(pm->ps->velocity[2] > 0 || jumpLevel == FORCE_LEVEL_5) {
					//holding jump in air
					...
					if ( ( curHeight<=forceJumpHeight[0] ||//still below minimum jump height
						...
						curHeight < forceJumpHeight[jumpLevel] && pm->ps->fd.forceJumpZStart)//still below maximum jump height
					{//can still go up
						...
						{//passed normal jump height  *2?
							if ( !(pm->ps->fd.forcePowersActive&(1<<FP_LEVITATION)) )//haven't started forcejump yet
							{
								...
								else if ( jumpLevel > FORCE_LEVEL_1 )
								{
						...
						//Lugormod
						if (jumpLevel == FORCE_LEVEL_5
							&& pm->ps->groundEntityNum == ENTITYNUM_NONE) {
								vec_t velocity = (forceJumpHeight[jumpLevel]-curHeight)/forceJumpHeight[jumpLevel]*forceJumpStrength[jumpLevel];//JUMP_VELOCITY;
								velocity /= 10;
						...
						{
							pm->ps->velocity[2] = (forceJumpHeight[jumpLevel]-curHeight)/forceJumpHeight[jumpLevel]*forceJumpStrength[jumpLevel];//JUMP_VELOCITY;
							pm->ps->velocity[2] /= 10;
					...
					else if ( curHeight > forceJumpHeight[0] && curHeight < forceJumpHeight[jumpLevel] - forceJumpHeight[0] )
				...
				//RoboPhred
					...
					//RoboPhred: Not jumping upwards but still jumping, lower our falling velocity.
					float velocity = pm->ps->gravity * forceJumpFallVelocityScaler[jumpLevel];
					if(pm->ps->velocity[2] < -velocity) {
						if(pm->ps->velocity[2] + (velocity * pml.frametime) > velocity)
							velocity = velocity - (pm->ps->velocity[2] + (velocity * pml.frametime));
						pm->ps->velocity[2] += velocity * pml.frametime;
#ifdef QAGAME
						//RoboPhred: is there an equivalent for pm?
						pm->ps->powerups[PW_SPEED] = level.time + 200;
#endif
					}
				}
				pm->cmd.upmove = 0;
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION) )
	{
		...
		{//on the ground
			...
			int anim = -1;
			float	vertPush = 0;
			if ( pm->cmd.rightmove > 0 && jumpLevel > FORCE_LEVEL_1 )
			{//strafing right
				if ( pm->cmd.forwardmove > 0 )
				{//wall-run
					if ( allowWallRuns )
			...
			else if ( pm->cmd.rightmove < 0 && jumpLevel > FORCE_LEVEL_1 )
			{//strafing left
				if ( pm->cmd.forwardmove > 0 )
				{//wall-run
					if ( allowWallRuns )
		...
		{//in the air
			...
			*/
			...
			{//run up wall, flip backwards
				...
				&& jumpLevel > FORCE_LEVEL_2//level 3 jump or better
	...
	*/
	if ( pm->ps->groundEntityNum == ENTITYNUM_NONE )
	{
		if (pm->cmd.upmove > 0 && jumpLevel >= FORCE_LEVEL_1)
		...
		{//in the air
				...
				&& !BG_InSpecialJump(pm->ps->legsAnim))
			{
				...
				{//there is a wall there
					...
					PM_SetAnim(parts, pm->cmd.rightmove > 0 ? BOTH_WALL_FLIP_RIGHT : BOTH_WALL_FLIP_LEFT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
...
static void PM_AirMove( void ) {
	...
	if (pm->ps->pm_type != PM_SPECTATOR)
	{
		...
		PM_CheckJump();
#else
		if (pm->ps->fd.forceJumpZStart && (pm->ps->forceJumpFlip ||	pm->ps->fd.forcePowerLevel[FP_LEVITATION] == FORCE_LEVEL_5))
		{
			PM_CheckJump();
		}
#endif
	...
	{//in a hovering vehicle, have air control
		if ( 1 )
		{
			...
			VectorScale( pm->ps->moveDir, pm->ps->speed, wishvel );
			VectorCopy( pm->ps->moveDir, wishdir );
			//EXPERIMENT (it was to scale with size)
			scale = 1.0f;
		}
#if 0
		else
	...
	else if (pm->ps->pm_type == PM_GRAPPLING){
		for ( i = 0 ; i < 2 ; i++ )
		{
			wishvel[i] = pml.forward[i]*fmove + pml.right[i]*smove;
		}
	...
	{ //reduced air control while not jetting
		...
		wishvel[2] = 0;
		if (//authenticated(&g_entities[pm->ps->clientNum], 
			//SUPER_JP_LEVEL) 
			pm->ps->userInt1
			&& pm->gametype == GT_FFA
			//&& !gameMode(GM_ANY)
			){
				//Lugormod better jetpacks for admins
...
static void PM_NoclipMove( void ) {
	...
	//pm->ps->viewheight = (int)((DEFAULT_VIEWHEIGHT - MINS_Z) * scaleh) + MINS_Z;
...
static int PM_TryRoll( void )
{
	...
	if ((pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE
		&& pm->ps->weapon != WP_STUN_BATON) || //Lugormod
		PM_IsRocketTrooper() ||
		BG_HasYsalamiri(pm->gametype, pm->ps) ||
		!BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
	{ //Not using saber, or can't use jump
#ifdef QAGAME //Lugormod mercenarys with the skill melee at lvl 2 can roll
...
static void PM_CrashLand( void ) {
	...
	//RoboPhred: ...why?  We dont have ducking in the air anymore anyway
	/*
	...
	*/
	...
	//Lugormod no falling damage when grappling
	//if (pm->ps->pm_type == PM_GRAPPLING) {
	//        return;
	// }
	...
	if ( !(pml.groundTrace.surfaceFlags & SURF_NODAMAGE) )  {
		if (delta > 7)
		{
			...
			if (pm->ps->fd.forceJumpZStart)
			{
				//RoboPhred: Not realistic, damage is speed, not jump point.
				//Actually, this is screwing up clients, reverting...

				//Forcepower benefit for falling will be added another way.
...
//RoboPhred: as PM_CheckDuck needs to know if we are on the ground, and PM_GroundTrace needs the mins, we move the CheckDuck min stuff to a new function
static void PM_SetBounds() {
	...
	{//riding a vehicle or are a vehicle
			...
			pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL))
		{
			...
			//pm->ps->viewheight = (int)((DEFAULT_VIEWHEIGHT - MINS_Z) * scaleh) + MAXS_Z;
			pm->ps->viewheight = DEFAULT_VIEWHEIGHT;
	...
	else
	{
		...
		//RoboPhred: Do this here, we need the maxs to be proper to avoid getitng stuck in low ceilings when crouching.
		else if (pm->ps->pm_flags & PMF_DUCKED)
		{
			pm->maxs[2] = pm->ps->crouchheight;//CROUCH_MAXS_2;
			//pm->ps->viewheight = (int)((CROUCH_VIEWHEIGHT - MINS_Z) * scaleh) + MINS_Z;
		...
		//Lugormod fix this
		else if (pm->ps->legsTimer 
			&& (pm->ps->legsAnim == BOTH_MEDITATE
			|| pm->ps->legsAnim == BOTH_CROUCH3
			|| pm->ps->legsAnim == BOTH_SIT6)) 
		{
			pm->maxs[2] = pm->ps->crouchheight;//CROUCH_MAXS_2;
			//pm->ps->viewheight = (int)((DEFAULT_VIEWHEIGHT - MINS_Z) * scaleh) + MINS_Z;
			pm->ps->viewheight = DEFAULT_VIEWHEIGHT;
...
static qboolean PM_CanStand() {
	...
	/* Original basejka way.  Bevel exploit is caused by this not finding bevels.

	VectorCopy(pm->maxs, maxs);
	maxs[2] = pm->ps->standheight;
	pm->trace (&trace, pm->ps->origin, pm->mins, maxs, pm->ps->origin, pm->ps->clientNum, pm->tracemask );
	if (trace.allsolid && trace.fraction < 1.0f)
	return qfalse;
	*/

	//Bevels are caught by linear traces, so trace the lower half of the bounding box up to the stand height.
	VectorCopy(pm->ps->origin, end);
	VectorCopy(pm->maxs, maxs);
	end[2] += pm->ps->standheight;
	maxs[2] = 0; //linear traces take the bounding box and drag it across the line, so we shouldn't have a maxs z
...
static qboolean PM_CheckDuck (void)
{
	...
	if (pm->ps->pm_flags & PMF_ROLLING)
	{
		...
		//RoboPhred
		if(!PM_CanStand()) {
			//We can't stand up, so keep ducking
			pm->ps->pm_flags |= PMF_DUCKED;
		}
		else {
			pm->ps->pm_flags &= ~PMF_ROLLING;
	...
	if (pm->ps->pm_flags & PMF_DUCKED)
	{
		//RoboPhred: used to set pm->maxs[2] here, which might have caused the bevel exploit.
		if(PM_CanStand()) {
...
void PM_FootSlopeTrace( float *pDiff, float *pInterval )
{
	...
	//Lugormod scale here?
	//VectorScale (footMins, scaleh, footMins);
	//VectorScale (footMaxs, scaleh, footMins);
...
static void PM_Footsteps( void ) {
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		...
		if ( ( (PM_RunningAnim( pm->ps->legsAnim )&&VectorLengthSquared(pm->ps->velocity)>=(40000 * scaleh)/*<- Lugormod scale fix*//*200*200*/) || PM_CanRollFromSoulCal( pm->ps ) ) &&
...
void PM_RocketLock( float lockDist, qboolean vehicleLock )
{
	...
	else
	{
		...
		VectorScale(WP_MuzzlePoint[WP_ROCKET_LAUNCHER], scaleh,
			muzzleOffPoint);//Lugormod
...
//---------------------------------------
{
	...
	//RoboPhred: For safty, a crash relied on this before.
...
static void PM_Weapon( void )
{
	...
	*/
	...
	if (duelInProgress(pm->ps) && pm->ps->emplacedIndex)
	{
		pm->ps->emplacedIndex = 0;
		pm->ps->saberHolstered = 0;
	}
...
#ifdef QAGAME
#ifdef LMD_NEW_WEAPONS
	amount = Weapon_FireEnergy(&g_entities[pm->ps->clientNum], pm->ps->weapon, pm->cmd.buttons & BUTTON_ALT_ATTACK);
#else
	...
	//RoboPhred: Shouldn't need to do the rest if we have no weapon (and fix potential issues while we're at it).
	if(pm->ps->weapon == WP_NONE)
		return;
...
#ifdef QAGAME
#ifdef LMD_NEW_WEAPONS
			addTime = Weapon_FireRate(&g_entities[pm->ps->clientNum], pm->ps->weapon, qfalse);
#else
			addTime = weaponData[pm->ps->weapon].fireTime;
#endif
#else
...
#ifdef QAGAME
#ifdef LMD_NEW_WEAPONS
			addTime = Weapon_FireRate(&g_entities[pm->ps->clientNum], pm->ps->weapon, qtrue);
#else
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	{//if running backwards while attacking, don't run as fast.
		//RoboPhred: saber styles, not force levels
		/*
					   ...
					   entityState_t *emplaced, int *crazySmoothFactor)
{
	...
	*/
	...
	{ //if using an emplaced gun, then we want to make sure we're angled to "hold" it right
		...
		/*
		else if (g_entities[emplaced->number].spawnflags&1024) {
		...
		//vec3_t gunangles;
		//AnglesSubtract(emplaced->apos.trBase, facingAngles, gunangles);
		//strap_G2API_SetBoneAngles(&(g_entities[emplaced->number].ghoul2), 0, "swivel_bone", gunangles , BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
...
{//																							up			right		fwd
	...
	strap_G2API_SetBoneAngles(ghoul2, 0, "cranium", cent_lerpAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
}
...
#endif
{
		...
		{ //dual stance attack
			...
			{ //one of the two jumps
				...
				{ //still on ground?
					...
					{
						//jump!
						pm->ps->velocity[2] = 250 * scaleh;//400;
				...
				//#ifndef __MACOS__ //Lugormod because of a bug in MrCpp
				else
		...
		{ //saberstaff attacks
			...
			{//one of the two jumps
				...
				{//still on ground?
					...
					if (pm->ps->legsAnim == BOTH_BUTTERFLY_LEFT)
					{
						pm->ps->velocity[2] = 350 * scaleh;
					}
					else
					{
						pm->ps->velocity[2] = 250 * scaleh;
					}
		...
		pm->ps->legsAnim == BOTH_JUMPATTACK7)
	{
		...
		/*
		//Lugormod (serverside only)
		# ifdef QAGAME
		if (newEvent == EV_JUMP) {
		gclient_t *client = &level.clients[pm->ps->clientNum];
		if (pm->ps->legsAnim == BOTH_JUMPATTACK7) {
		trap_SendServerCommand( 
		pm->ps->clientNum,
		"print \"flipsinarow +1\n\"");

		if (client->flipsinarow++ > 1) {
		trap_SendServerCommand( 
		pm->ps->clientNum,
		va("cp \"%i\"",
		client->flipsinarow));
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd)
{
	...
	//Lugormod is that really correct???
...
qboolean PM_WeaponOkOnVehicle( int weapon )
{
	...
	case WP_SABER:
	case WP_BLASTER:
		////RoboPhred: aww, clients need this as well...
		//case WP_BRYAR_PISTOL:
		//case WP_THERMAL:
		return qtrue;
		break;
...
//RoboPhred: This has always delt with server side entities, now its dealing with logical entities
#ifdef QAGAME
...
void PM_VehForcedTurning(bgEntity_t *veh)
{
	...
	gentity_t *dst = GetEnt(veh->playerState->vehTurnaroundIndex);
										...
										void (*localTrace)(trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask))
{
	...
	{//only those types of vehicles have dynamic bboxes, the rest just use a static bbox
		VectorSet( maxs, veh->m_pVehicleInfo->width/2.0f, veh->m_pVehicleInfo->width/2.0f, veh->m_pVehicleInfo->height+DEFAULT_MINS_2); //* scaleh);
		VectorSet( mins, veh->m_pVehicleInfo->width/-2.0f, veh->m_pVehicleInfo->width/-2.0f, DEFAULT_MINS_2);// * scaleh);
		VectorScale(maxs, scaleh, maxs);//Lugormod
		VectorScale(mins, scaleh, mins);//Lugormod
		return;
	}
	else
	{
		...
		VectorMA( origin, scaleh * veh->m_pVehicleInfo->length/2.0f, axis[0], point[0] );
		VectorMA( origin, scaleh * -veh->m_pVehicleInfo->length/2.0f, axis[0], point[1] );
		//extrapolate each side up and down
		VectorMA( point[0], scaleh * veh->m_pVehicleInfo->height/2.0f, axis[2], point[0] );
		VectorMA( point[0], scaleh * -veh->m_pVehicleInfo->height, axis[2], point[2] );
		VectorMA( point[1], scaleh * veh->m_pVehicleInfo->height/2.0f, axis[2], point[1] );
		VectorMA( point[1], scaleh * -veh->m_pVehicleInfo->height, axis[2], point[3] );

		VectorMA( origin, scaleh * veh->m_pVehicleInfo->width/2.0f, axis[1], point[4] );
		VectorMA( origin, scaleh * -veh->m_pVehicleInfo->width/2.0f, axis[1], point[5] );
		//extrapolate each side up and down
		VectorMA( point[4], scaleh * veh->m_pVehicleInfo->height/2.0f, axis[2], point[4] );
		VectorMA( point[4], scaleh * -veh->m_pVehicleInfo->height, axis[2], point[6] );
		VectorMA( point[5], scaleh * veh->m_pVehicleInfo->height/2.0f, axis[2], point[5] );
		VectorMA( point[5], scaleh * -veh->m_pVehicleInfo->height, axis[2], point[7] );
		/*
...
void PmoveSingle (pmove_t *pmove) {
		...
		|| endAnim(pm->ps->legsAnim))
	{
		...
		pm->cmd.forwardmove = 0;
		pm->cmd.buttons = 0;
	} else if ((emote = emoteAnim(pm->ps->torsoAnim))->name
		|| (emote = emoteAnim(pm->ps->legsAnim))->name) {
			...
			if (emote->animEnd != -1
				...
				|| pm->cmd.upmove ))
			{//End moves
				PM_SetAnim ( emote->setanim & SETANIM_BOTH, emote->animEnd, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			} else {
				if (emote->animEnd != -1) {
					...
					if (emote->setanim & SETANIM_TORSO
	...
	//#endif //QAGAME
	if ( (pm->ps->legsAnim == BOTH_FORCE_RAGE)
		&& (pm->ps->legsTimer > 0) )
	{
		stiffenedUp = qtrue;
		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
	...
	{
		//Lugormod not sure if there is a better way.
		if ( pm->ps->legsTimer < 100 )
		{
			PM_SetAnim( SETANIM_BOTH, BOTH_HUGGERSTOP1, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			pm->ps->legsTimer += 200;

		} else {
			PM_ContinueLegsAnim(BOTH_HUGGER1);
			stiffenedUp = qtrue;
			PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
			//pm->ps->saberLockTime = level.time + 100;
			pm->cmd.rightmove = 0;
	...
	}else if ( pm->ps->legsAnim == BOTH_HUGGEE1 )
	{
		if ( pm->ps->legsTimer < 100 )
		{
			PM_SetAnim( SETANIM_BOTH, BOTH_HUGGEESTOP1, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			pm->ps->legsTimer += 200;

		} else {
			PM_ContinueLegsAnim(BOTH_HUGGEE1);
			stiffenedUp = qtrue;
			PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
			//pm->ps->saberLockTime = level.time + 100;
			pm->cmd.rightmove = 0;
			...
			pm->cmd.forwardmove = 0;
			pm->cmd.buttons = 0;
		}//not so nice dubbeling that ...

	} else if ( pm->ps->legsAnim == BOTH_HUGGERSTOP1 || pm->ps->legsAnim == BOTH_HUGGEESTOP1)
	{
		//pm->ps->saberLockTime = level.time + 100;

		if ( pm->ps->legsTimer < 200 )
		{
			vec3_t dir;
			AngleVectors(pm->ps->viewangles, dir, 0, 0);
			dir[2] = 0;
	...
	*/
		...
		&&pm->ps->m_iVehicleNum)
	{
		...
		//RoboPhred: the target is only ever client side anyway, and now its a logical ent as well
#ifdef QAGAME
		else if (veh && veh->playerState
			&& veh->playerState->vehTurnaroundIndex
			&& veh->playerState->vehTurnaroundTime > pm->cmd.serverTime
			//Lugormod only pilot
			//&& veh->m_pVehicle->m_pPilot->s.number == pm->ps->clientNum
			)
	...
	*/
	...
	if ( pm->ps->pm_type >= PM_DEAD  && pm->ps->pm_type < PM_GRAPPLING) {
		pm->cmd.forwardmove = 0;
		pm->cmd.rightmove = 0;
		pm->cmd.upmove = 0;
	}
	...
	*/
	...
	//RoboPhred: new function for this
	PM_SetBounds();

	//RoboPhred: moved up here to get the ground entity number for PM_CheckDuck();
	// set groundentity
	PM_GroundTrace();
	if ( pm_flying == FLY_HOVER )
	...
	if(PM_CheckDuck())
		PM_SetBounds(); //update the bounds from a change to ducking

	vec_t vel;

	if (pm->ps->pm_type == PM_GRAPPLING) {
		//Lugormod
		if ((vel = DotProduct(pm->ps->vehOrientation, pm->ps->velocity)) <= 0) {

			int offset = 5;
			//Don't drop if we are lower than 2048 from the connection point.
			if(pm->cmd.upmove < 0 && Distance (pm->ps->origin, pm->ps->hyperSpaceAngles) + pm->ps->vehSurfaces < 2048)
				offset = -450;
			VectorMA(pm->ps->velocity, -vel + offset, pm->ps->vehOrientation, pm->ps->velocity);       
		}
		//RoboPhred: dont let them raise faster than they can walk to stop assisted bunnyhopping.
		if (pm->cmd.upmove > 0 && VectorLength(pm->ps->velocity) < 175) {
			VectorMA (pm->ps->velocity, (pm->ps->gravity + 500) * pml.frametime, pm->ps->vehOrientation, pm->ps->velocity);
		}
		//Air friction
		//VectorScale(pm->ps->velocity, 0.9f, pm->ps->velocity);
		pm->cmd.upmove = 0;
		//pm->cmd.forwardmove = 0;`
		//pm->cmd.rightmove = 0;
	...
	//RoboPhred: Moved above jetpack/grapple code, we need the ground trace there.
	/*
	// set groundentity
	PM_GroundTrace();
	if ( pm_flying == FLY_HOVER )
	...
	*/
	...
	{ //we are a vehicle
		...
		if (veh->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER 
			/*&& veh->m_pVehicle->m_pVehicleInfo->type != VH_SPEEDER*/)
...
void Pmove (pmove_t *pmove) {
	...
	int i;
	for (i = 0;i < 8;i++) {
		if (pmove->ps->stats[STAT_EXTRA_FORCE_BITS]&(1 << i)) {
			pmove->ps->fd.forcePowerLevel[i] |= 4;
		}
	}

	for (i = 8;i < 16;i++) {
		if (pmove->ps->stats[STAT_EXTRA_FORCE_BITS2]&(1 << (i - 8))) {
			pmove->ps->fd.forcePowerLevel[i] |= 4;
		}
	}
	...
	while ( pmove->ps->commandTime != finalTime ) {
		...
		PmoveSingle( pmove );
		pm->ps->viewheight *= scaleh; //Lugormod scale
		pm->ps->userInt2 = (pm->ps->viewheight >> 8); //Lugormod send one more bit
