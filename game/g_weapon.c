...
//RoboPhred
#define REPEATER_ALT_SPLASH_RADIUS		80
...
//RoboPhred: still used for entity shooter_blaster, need to make that use misc_weapon_shooter and call real fire code.
//---------------------------------------------------------
...
//---------------------------------------------------------
{
	...
	if ( g_gametype.integer == GT_SIEGE ||
		g_gametype.integer == GT_BATTLE_GROUND)	// we've been having problems with this being too hyper-potent because of it's radius
	{
...
static void WP_DEMP2_MainFire( gentity_t *ent ){
	...
	int life = 10000; 
	missile = CreateMissile(muzzle, forward, velocity, life, ent, qfalse);
...
void DEMP2_AltRadiusDamage( gentity_t *ent ){
	...
	//RoboPhred: what the hell?  THis makes our min 200
	if (fact > 1)
		//if (fact < 1)
		fact = 1;
...
//---------------------------------------------------------
{
	...
	//RoboPhred: temp for testing
	/*off now
	int scale = trap_Cvar_VariableIntegerValue("lmd_experemental_demp2altfirescale");
	if(authenticated(&g_entities[ent->r.ownerNum], 1) && scale > 0){
		ent->count = scale;
	}
	*/
...
//---------------------------------------------------------
{
	//RoboPhred: restructured most of this for tech.  Also cleaned up the basejka code
	int damage;
	...
	if(count > 3)
		...
		count = 1;
		damage = 1; //this was just a tap-fire
	}
	...
	missile->splashRadius = DEMP2_ALT_SPLASHRADIUS; //this is limited in the Demp2_AltRadiusDamage function (I think)
...
//---------------------------------------------------------
{
	...
	else
	{
		...
		if (G_RadiusDamage( ent->r.currentOrigin, ent->parent,  ent->splashDamage, ent->splashRadius, 
			ent, ent, ent->splashMethodOfDeath) && ent->r.ownerNum < MAX_GENTITIES && g_entities[ent->r.ownerNum].client)
		{
			g_entities[ent->r.ownerNum].client->accuracy_hits++;
			//RoboPhred
			PlayerAcc_Stats_SetHits((&g_entities[ent->r.ownerNum]), PlayerAcc_Stats_GetHits((&g_entities[ent->r.ownerNum])) + 1);
		}
...
{
	//Ufo:
	if (other && other->s.number < ENTITYNUM_WORLD && Q_stricmp("misc_model_breakable", other->classname) && Q_stricmp("func_static", other->classname))
...
void proxMineThink(gentity_t *ent)
{
	...
	{ //eh, just check for clients, don't care about anyone else...
		...
		{
			//Ufo: exclude buddies or friends
			if (isBuddy(owner, cl) || PlayerAcc_Friends_IsFriend(owner, PlayerAcc_GetId(cl)))
			{
				i++;
				continue;
			}
			...
			//Ufo: 
			if (cl->client->pers.Lmd.persistantFlags & SPF_IONLYDUEL || !CanDamage(cl, ent->r.currentOrigin))
			{
				i++;
				continue;
			}
...
void charge_stick (gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	self->nextthink = level.time + 30000; //Lugormod gotta go higher
	if (g_gametype.integer != GT_SIEGE 
		/*&& g_gametype.integer != GT_BATTLE_GROUND*/) {
			self->nextthink = level.time + 300000;
	}
...
void       DismembermentTest (gentity_t *self); //Lugormod
			...
			if ( render_impact )
			{
					...
					|| traceEnt->s.eType == ET_MOVER )
				{
					...
					if ( traceEnt->client
						...
						&& !meditateProtect(traceEnt) 
						&& !(traceEnt->client->pers.Lmd.persistantFlags & SPF_IONLYDUEL)
						&& (!duelInProgress(&traceEnt->client->ps)
						|| traceEnt->client->ps.duelIndex == ent->s.number)) 
...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	if (g_grapplingHook.integer == 2 || (
		g_grapplingHook.integer 
		&& (alt_fire 
		|| (ent->client && ent->client->hook)))) {
			Merc_FireGrapplingHook (ent, alt_fire);
			return;
	}
#endif
	...
	{ //see if either party is involved in a duel
		...
		if (duelInProgress(&tr_ent->client->ps) &&
...
void WP_FireMelee( gentity_t *ent, qboolean alt_fire )
{
	...
	{ //hit something
		...
		{ //damage them, do more damage if we're in the second right hook
			...
			if (gameMode(GM_BOXING_ARENA)) {
				dmg *= 4;
			} else if ((skill = PlayerProf_Merc_GetMeleeSkill(ent))) {
				dmg += 0.1 * dmg * skill;
			}
...
void CalcMuzzlePoint ( gentity_t *ent, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) 
{
	...
	if (ent && ent->client && ent->client->ps.iModelScale) { //Lugormod
		VectorScale(WP_MuzzlePoint[weapontype], 
...
gentity_t *WP_FireVehicleWeapon( gentity_t *ent, vec3_t start, vec3_t dir, vehWeaponInfo_t *vehWeapon, qboolean alt_fire, qboolean isTurretWeap )
{
	...
	{//projectile entity
		...
		*/
		...
		//Lugormod Don't want them to stay around that long
		//if ( vehWeapon->iLifeTime > 60000) {
		//        vehWeapon->iLifeTime = 60000;
		//}
		...
		|| ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER)
	{
		...
		{//for some reason, the walker always draws the crosshair out from from the first muzzle point
			...
			if (ent->m_pVehicle->m_pPilot) {
				AngleVectors( ((gentity_t*)ent->m_pVehicle->m_pPilot)->client->ps.viewangles, dir, NULL, NULL );
			} else { //Don't know who would fire if there's no
				//pilot, but what ever.
			...
			for ( i = 0; i < MAX_VEHICLE_MUZZLES; i++ )
			{
				...
				if ( pVeh->m_iMuzzleTag[i] != -1 && pVeh->m_iMuzzleWait[i] < level.time )
				{
					...
					{//have enough ammo to shoot
						...
						//       } else {//Lugormod
						//        missile = WP_FireVehicleWeapon( ent, pVeh->m_vMuzzlePos[i], pVeh->m_vMuzzleDir[i], vehWeapon, alt_fire, qfalse );
						//}
...
#ifdef LMD_NEW_WEAPONS
qboolean Weapon_Fire(gentity_t *ent, weapon_t weapon, qboolean altFire);
#endif
void Merc_FireGrapplingHook( gentity_t *ent, qboolean alt_fire );
...
void FireWeapon( gentity_t *ent, qboolean altFire ) {
	...
	if( ent->s.weapon != WP_SABER && ent->s.weapon != WP_STUN_BATON && ent->s.weapon != WP_MELEE ) 
	{
		...
		if( ent->s.weapon == WP_FLECHETTE ) {
			...
			//RoboPhred
			PlayerAcc_Stats_SetShots(ent, PlayerAcc_Stats_GetShots(ent) + FLECHETTE_SHOTS);
		} else {
	...
	else
	{
...
#ifdef LMD_NEW_WEAPONS
		if(Weapon_Fire(ent, ent->s.weapon, altFire) == qtrue) {
		...
		case WP_CONCUSSION:
#ifndef LMD_NEW_WEAPONS
			if ( altFire || gameMode(GM_INSTGIB) || g_gametype.integer == GT_GHOST)
			{
...
#else
			Merc_FireGrapplingHook(ent, altFire);
#endif
			break;
...
void emplaced_gun_use( gentity_t *self, gentity_t *other, trace_t *trace )
{
	...
	if(PlayerUseableCheck(self, activator) == qfalse)
		return;
	/*
	if (activator->client->ps.trueJedi) {
		//Lugormod trueJedi can't use this
		return;
	}
	*/
	...
	if (g_gametype.integer == GT_JEDIMASTER 
		&& activator->client->ps.isJediMaster)
...
void emplaced_gun_update(gentity_t *self)
{
	...
	{ //we are dead, see if it's time to respawn
		...
		self->health = EMPLACED_GUN_HEALTH*0.4; //Why *0.4??
		self->s.health = self->health;
	}
...
void SP_emplaced_gun( gentity_t *ent )
{
	...
	char *name;
	if (g_gametype.integer == GT_FFA && g_dontLoadNPC.integer && Q_stricmp("gun", ent->classname + 9) == 0) {
			ent->spawnflags |= 1024;
			name = "models/map_objects/imp_mine/turret_chair.glm";
			VectorSet( ent->r.mins, -30, -30, 0 );
			VectorSet( ent->r.maxs, 30, 30, 70 );
	} else {
		name = "models/map_objects/mp/turret_chair.glm";
