...
extern vmCvar_t		g_backslashDamageScale;
...
float RandFloat(float min, float max) {
	...
	//Lugormod well, that didn't work
	return (Q_random(&randseed) * (max - min)) + min;
}
...
qboolean G_CanBeEnemy(gentity_t *self, gentity_t *enemy)
{
	...
	if (duelInProgress(&self->client->ps) && self->client->ps.duelIndex != enemy->s.number)
	{ //dueling but not with this person
		return qfalse;
	}

	//if (enemy->client->ps.duelInProgress && enemy->client->ps.duelIndex != self->s.number)
	if (duelInProgress(&enemy->client->ps) && enemy->client->ps.duelIndex != self->s.number)
	{ //other guy dueling but not with me
		//return qfalse;
		//Lugormod but get away from my ship!!
		if (enemy->client->ps.groundEntityNum != 
			self->s.number) {
				return qfalse;
		}
	}
	...
	if ( meditateProtect(enemy)) {
		//meditating won't protect you if you're sitting on a ship.
		if (enemy->client->ps.groundEntityNum != 
			self->s.number) {
				return qfalse;
		}
...
static GAME_INLINE void SetSaberBoxSize(gentity_t *saberent)
{
	...
	{
		if (!owner)
			assert(!"Saber with no owner?"); //Lugormod this has triggered
		else if (!owner->inuse)
			assert(!"Owner not inuse"); //Lugormod this has triggered
		else if (!owner->client)
			assert(!"Owner with no client"); //Lugormod this has triggered
		return;
	}
...
static GAME_INLINE qboolean WP_SabersCheckLock2( gentity_t *attacker, gentity_t *defender, sabersLockMode_t lockMode )
{
	...
	G_SetAnim(attacker, SETANIM_BOTH, attAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
	...
	G_SetAnim(defender, SETANIM_BOTH, defAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
...
qboolean WP_SabersCheckLock( gentity_t *ent1, gentity_t *ent2 )
{
	...
	{ //can always get into locks with NPCs
		...
		if (!duelInProgress(&ent1->client->ps) ||
			!duelInProgress(&ent2->client->ps) ||
...
void WP_SaberApplyDamage( gentity_t *self )
{
	...
	for ( i = 0; i < numVictims; i++ )
	{
		...
		//Ufo: why does saber throw apply damage from trace and the throw itself?
			...
			else
			{
				...
				else
				{
					...
					{ //I suppose I could tie this into the saberblock event, but I'm tired of adding flags to that thing.
						...
						//RoboPhred: attempt to avoid crashes
						if(!teS)
							return;
						...
						{//still alive
							...
							{//close enough and knockback high enough to possibly knock down
								...
								{//within range of my fist or within ground-shaking range and not in the air
									G_Knockdown( radiusEnt, 0);//, ent, entDir, 500, qtrue );
								}
							}
						}
					}
...
static GAME_INLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	{ //this animation is that of the last attack movement, and so it should do full damage
		...
		if ( d_saberSPStyleDamage.integer )
		{
			...
			//Lugormod make rage better (useful at all)
			//RoboPhred: We want this always, no g_fixForce.  Reverting.
			//Ufo: I'd rather g_fixForce but wasn't this supposed to use fDmg rather than dmg?
			if ( (g_fixForce.integer & (1 << FP_RAGE)) && fDmg > 0.1f )
			{
				if ( (self->client->ps.fd.forcePowersActive & (1<<FP_RAGE)) )
				{//add some damage if raged
					fDmg *= 1.0f + (self->client->ps.fd.forcePowerLevel[FP_RAGE] / 5.0f);
					//dmg += self->client->ps.forcePowerLevel[FP_RAGE] * 5.0f;
				}
				/*
				else if ( self->client->ps.fd.forceRageRecoveryTime >= level.time )
	...
	//JAPRO - Serverside - Japro Saber Cleanup MP saber damages - Start
		...
		else if (self->client->ps.fd.saberAnimLevel == SS_STAFF)//Staff Style
		{
			if (self->client->ps.saberMove == LS_SPINATTACK)//Staff crouch kata (auto ramped)
				...
				dmg = G_GetAttackDamage(self, 2, 25, 0.5f);
			else if (self->client->ps.saberMove == LS_STABDOWN_STAFF)//Tweaked staff groundstab
		...
		else if (self->client->ps.fd.saberAnimLevel == SS_DUAL)//Dual Style
		{
			...
			else if (self->client->ps.saberMove == LS_STABDOWN_DUAL)//Tweaked dual groundstab
				dmg = G_GetAttackDamage(self, 2, 30, 0.5f);
		...
		else if (self->client->ps.saberMove == LS_STABDOWN)//Tweaked single groundstab (all styles)
			dmg = G_GetAttackDamage(self, 2, 20, 0.5f);
		...
		else if (self->client->ps.fd.saberAnimLevel == SS_STRONG)//Red Style 
		{
			if (self->client->ps.saberMove == LS_A_T2B) { //Red Vert
					...
					dmg = G_GetAttackDamage(self, 2, 70, 0.65f);
			}
	...
	if (dmg > SABER_NONATTACK_DAMAGE)
	{
		...
		//RoboPhred
		/*
		4, if possible saber dmg increase with level so the higher the level you 
		are the more dmg ur saber will do (for example if u did 1 slash @ lvl 1 and 
		it caused 20hp dmg @ lvl 40 the same attack would cause 40hp dmg) also with this 
		feature u must have lvl 5 saber attack or u dont get increased dmg 
		*/
		/*temp disabled I guess.
		if(PlayerAcc_Prof_GetSkill(self, PROF_JEDI, FP_SABER_OFFENSE) == FORCE_LEVEL_5){
			int inc = 0.5 * (Accounts_Prof_GetLevel(self) - 1);
			if(inc > 25)
				inc = 25;
			dmg += inc;
			//dmg += (0.5 * (self->client->sess.level - 1));
		}
		*/
	...
	if (g_gametype.integer == GT_JEDIMASTER 
		&& dmg > SABER_NONATTACK_DAMAGE
		&& self->client->ps.isJediMaster)
	{ //give the Jedi Master more saber attack power
		dmg *= g_jmsaberDamageScale.value;
	}
		...
		*/
		{
			...
			*/
			...
			{ //set anim now on both parts
				...
				G_SetAnim(self, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
			}
	...
	{//hit something that had health and takes damage
		...
		if (g_entities[tr.entityNum].client &&
			...
			duelInProgress(&g_entities[tr.entityNum].client->ps) &&
	...
	{ //saber clash
		...
		if (duelInProgress(&otherOwner->client->ps) &&
			otherOwner->client->ps.duelIndex != self->s.number)
		{
			return qfalse;
		}

		if (duelInProgress(&self->client->ps) &&
...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) 
	{
		...
		//Ufo: this is just plain annoying
		if (ent && self->client->ps.duelInProgress && ent->r.ownerNum != self->client->ps.duelIndex)
			continue;
...
static GAME_INLINE qboolean CheckThrownSaberDamaged(gentity_t *saberent, gentity_t *saberOwner, gentity_t *ent, int dist, int returning, qboolean noDCheck)
{
	...
	{ //hit a client
		...
		if (ent->inuse && ent->client &&
			...
			duelInProgress(&saberOwner->client->ps) &&
...
//is Auths_Inferior, or equal and a random factor is met, then the saber will be tossed to the ground.
qboolean saberCheckKnockdown_Thrown(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other)
{
	int throwLevel = 0;
	int defenLevel = 0;
...
void UpdateClientRenderinfo(gentity_t *self, vec3_t renderOrigin, vec3_t renderAngles)
{
	...
	//RoboPhred: sanity checking, there is a crash here somewhere...
	//assert(self->s.number < MAX_CLIENTS && self->localAnimIndex <= 1);
...
static void G_GrabSomeMofos(gentity_t *self)
{
	...
	trap_G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt, &boltMatrix, flatAng, self->client->ps.origin, level.time, NULL, self->modelScale);
	...
	if (PlayerProf_Merc_GetMeleeSkill(self) > 4 && (trace.fraction == 1.0f || trace.entityNum == ENTITYNUM_WORLD)) {
		trap_G2API_GetBoltMatrix(self->ghoul2, 0, ri->handLBolt, &boltMatrix, flatAng, self->client->ps.origin, level.time, NULL, self->modelScale);
		BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, pos);
		trap_G2Trace( &trace, self->client->ps.origin, grabMins, grabMaxs, pos, self->s.number, MASK_SHOT, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
	}
		...
		{ //grabbed an active player/npc
			...
			if (tortureAnim == -1 || correspondingAnim == -1)
			{
				...
				{ //you failed to grab anyone, play the "failed to grab" anim
					G_SetAnim(self, SETANIM_BOTH, BOTH_KYLE_MISS, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
			...
			G_SetAnim(self, SETANIM_BOTH, tortureAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
			if (self->client->ps.torsoAnim == tortureAnim)
			{ //providing the anim set succeeded..
				self->client->ps.weaponTime = self->client->ps.torsoTimer;
			}

			G_SetAnim(grabbed, SETANIM_BOTH, correspondingAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
		...
		else
		{
			...
			else
			{
				...
				{ //grapplee
					...
					else if (grappler->client->ps.torsoAnim == BOTH_KYLE_PA_1)
					{ //knee-throw
						idealDist = 34.0f;
					} else {
						idealDist = 48.0f;
				...
				{ //grappler
					...
					{ //grab punch
						if (self->client->grappleState == 1)
						{ //smack
							if (self->client->ps.torsoTimer < 3000)
							{
								int grapplerAnim = grappler->client->ps.torsoAnim;
								int grapplerTime = grappler->client->ps.torsoTimer;
						...
						{ //smack!
							if (self->client->ps.torsoTimer < 1800)
							{
								int grapplerAnim = grappler->client->ps.torsoAnim;
								int grapplerTime = grappler->client->ps.torsoTimer;
						...
						{ //SMACK!
							if (self->client->ps.torsoTimer < 800)
							{
								vec3_t tossDir;
								...
								{ //if still alive knock them down
									grappler->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
									grappler->client->ps.forceDodgeAnim = BOTH_PLAYER_PA_FLY;
									G_SetAnim(grappler, SETANIM_BOTH, BOTH_PLAYER_PA_FLY, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
									grappler->client->ps.forceHandExtendTime = level.time + 1300;
								}
							}
						}
		...
		if (self->client->ps.saberLockTime > level.time && self->client->ps.saberEntityNum)
		{
			...
			while (rSaberNum < MAX_SABERS)
			{
				...
				{ //Don't bother updating the bolt for each blade for this, it's just a very rough fallback method for during saberlocks
					...
					//Lugormod it was using saberNum rather
					//than rSaberNum which seems weird
					//got a crash here:
